       ...
       match config with
        | (cs, stack, ((st, i, o) as cfg)) ->
          (match x with
           | BINOP bo ->
             (match stack with
              | (r :: l :: ls) -> eval env (cs, Expr.eval st (Binop (bo, Const l, Const r)) :: ls, cfg) xs
           ...
           | BEGIN (args, locs)  -> 
             ...
             let fstate, fstack = addVal (State.push_scope st (args @ locs)) args stack in
             eval env (cs, fstack, (fstate, i, o)) xs
           | CALL name  -> 
             eval env ((xs, st) :: cs, stack, (st, i, o)) (env#labeled name)
           | END  -> 
             (match cs with
              | []               -> config
              | ((p, st') :: cs) -> eval env (cs, stack, (State.drop_scope st st', i, o)) p
             )
          )
    )
...
let rec compileDefinition label_generator = 
  ...
  | (name, (args, locs, body)) -> let body, new_gen = compile' label_generator body in
                                  [LABEL name; BEGIN (args, locs)] @ body @ [END], new_gen
